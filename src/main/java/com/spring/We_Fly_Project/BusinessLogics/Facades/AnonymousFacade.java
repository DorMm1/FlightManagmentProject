...
public class AnonymousFacade extends BaseFacade{
    ...
    public boolean add_customer(CustomerPOCO new_customer){
        if(new_customer.first_name.contains("123456789~!#$%^&*(){}[]?><:;|\"") || new_customer.last_name.contains("123456789~!#$%^&*(){}[]?><:;|\"")){
            System.out.println("invalid name");
            return false;
        }
        if(new_customer.phone_no.length()!=10){
            System.out.println("phone number must contain 10 digits");
            return false;
        }
        customersDAO.add(new_customer);
    ...
    public UserPOCO create_new_user_account(String username, String password, String email){
        ...
        if(!isValidEmail(email) || email.equals("t@t.com")){ // email validation
